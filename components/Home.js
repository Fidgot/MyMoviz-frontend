...
function Home() {
  ...
  // Movies list
  useEffect(() => {
    fetch('https://my-moviz-backend-nu.vercel.app/movies')
    .then(response => response.json())
    .then(data => {
      const formatedData = data.movies.map(movie => {
        const poster = `https://image.tmdb.org/t/p/w500/${movie.poster_path}`;
        const formatedOverview = "";
        if (movie.overview.length > 200) {
          formatedOverview = movie.overview.substring(0,200) + " ..."; 
        } else {
          formatedOverview = movie.overview
        }
        return { title: movie.title, poster, voteAverage: movie.vote_average, voteCount: movie.vote_count, overview: formatedOverview };
      });
